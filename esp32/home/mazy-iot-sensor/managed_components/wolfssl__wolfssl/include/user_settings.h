...
/* #define SINGLE_THREADED */  /* DISABLED: project runs FreeRTOS — enabling
                                  this removes mutex protection from the global
                                  WC_RNG and causes Ed25519 key generation to
                                  fail under concurrent task access. */
 ...
 * Uncomment lines to force SW instead of HW acceleration */
    ...
    #define WOLFSSL_ESP32
    /* ESP32-C3 is RISC-V. The hardware crypto accelerator requires matching
     * SP (Single-Precision) RISC-V math support to initialise correctly.
     * Without WOLFSSL_SP_RISCV32, wc_InitRng() faults during hw accel init
     * and Ed25519 key generation fails entirely. Disable all HW crypto and
     * let wolfSSL use its software implementations instead — correct and
     * fast enough for HomeKit's one-time key generation at boot.
     *
     * Note: sha256.c guards its ESP32 hw-path with NO_WOLFSSL_ESP32_CRYPT_HASH
     * rather than NO_ESP32_CRYPT, so both flags are required here. */
    #define NO_ESP32_CRYPT
    #define NO_WOLFSSL_ESP32_CRYPT_HASH        /* disables SHA HW in sha256.c */
    #define NO_WOLFSSL_ESP32_CRYPT_AES
    #define NO_WOLFSSL_ESP32_CRYPT_RSA_PRI

    /* RISC-V SP math: required for correct Ed25519/Curve25519 on C3 core */
    #define WOLFSSL_SP_MATH_ALL
    #define WOLFSSL_SP_RISCV32

    /* No SHA384/SHA512 hardware on C3 (unchanged) */
